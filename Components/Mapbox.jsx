...
const Mapbox = () => {
  ...
  const classifyData = (data) => {
    ...
    data.features.forEach((feature) => {
      ...
      const proposedTax =
        feature.properties.property_value * 0.01177 * propTaxFactor +
        feature.properties.lot_area * areaTaxFactor;
    ...
    data.features.forEach((feature) => {
      ...
      const targetBlend =
        (feature.properties.tax_difference + largestDifference) /
        (largestDifference * 2);
      if (targetBlend > 0.5) {
        feature.properties.color = blend_colors(
          "#00ffff",
          "#ff00ff",
          targetBlend * 2 - 1
        );
    ...
    data.features = data.features.filter(
      (feature) => feature.properties.tax_difference !== undefined
    );
    return data;
  };
  ...
  useEffect(() => {
    ...
    <strong>2022 Total Assessed Property Value:</strong> $${
      properties.property_value || "N/A"
    }<br>
    <strong>Tax Difference:</strong> $${
...
`;
          ...
          map.on("click", "plot-boundary", (e) => {
            ...
            // Create a new popup and set it as the active popup
    ...
    // Listen for clicks on the map. If the clicked feature is not a 'plot-boundary', then remove the active popup
    map.on("click", (e) => {
